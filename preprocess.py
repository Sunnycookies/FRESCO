...
class Preprocess(nn.Module):
    def __init__(self, device, config) -> None:
        ...
        self.vae = AutoencoderKL.from_pretrained("stabilityai/sd-vae-ft-mse", torch_dtype=torch.float16, local_files_only=True)
        self.pipe = StableDiffusionPipeline.from_pretrained(config['sd_path'], vae=self.vae, torch_dtype=torch.float16,local_files_only=True)
