...
import torchvision
import diffusers
from diffusers import StableDiffusionPipeline, AutoencoderKL, DDPMScheduler, ControlNetModel, DDIMScheduler, DDIMInverseScheduler
import numpy as np
...
def get_models(config):
    ...
    # diffusion model
    vae = AutoencoderKL.from_pretrained("stabilityai/sd-vae-ft-mse", torch_dtype=torch.float16, local_files_only=True)
    pipe = StableDiffusionPipeline.from_pretrained(config['sd_path'], vae=vae, torch_dtype=torch.float16,local_files_only=True)
    # pipe.scheduler = DDPMScheduler.from_config(pipe.scheduler.config)
    
    pipe.scheduler = DDIMScheduler.from_pretrained(config['sd_path'] ,subfolder="scheduler", local_files_only=True)
...
def run_keyframe_translation(config):
    ...
    # # TEST pnp
    # a_prompt = ''
    # n_prompt = 'ugly, blurry, black, low res, unrealistic'
    ...
    for i in range(frame_num):
        ...
        * Turn off background smoothing: set saliency = None in apply_FRESCO_opt()
        ...
        if config['edit_mode']=='pnp':
            frescoProc_pnp = apply_FRESCO_attn(pipe, 'pnp')
            frescoProc_pnp.controller.enable_controller(interattn_paras=interattn_paras, attn_mask=attn_mask)
            frescoProc_pnp.controller.set_qk_injection_timesteps(qk_injection_timesteps)
        ...
        apply_FRESCO_opt(pipe, steps = timesteps[:config['end_opt_step']],
                         ...
                         saliency=saliency, optimize_temporal = True, edit_mode=config['edit_mode'])
        
        # TEST: turn off all fresco support turn on cfa
        # attn mask: list of tensor  [ [frames, 4096], [frames. 1024], [frames, 256] ]
        # frescoProc.controller.disable_controller() 
        # disable_FRESCO_opt(pipe)
        # if config['edit_mode']=='pnp':
        #     pass
            # disable_FRESCO_opt(pipe)
            # frescoProc_pnp.controller.disable_controller() 
        ...
        if config['edit_mode'] == 'pnp':
            ...
            pnp_prompt_embeds = pipe._encode_prompt(
                [config['inv_prompt']] * len(img_idx),
                device,
                1,
                False,
                '',
            ...
            prompt_embeds = torch.cat([pnp_prompt_embeds, prompt_embeds])
            
            
            
        else:
            inv_noise = load_source_latents_t(timesteps[0],config['inv_latent_path'])

        gc.collect()
        torch.cuda.empty_cache()
        # ADD by BYH
        # edit mode with pnp edit.   
        if config['edit_mode'] == 'pnp': 
            latents = inference_pnp(pipe, frescoProc_pnp, 
        ...
        # Deprecated
        # run!
        # Edit mode with DDIM sampling SDEdit(and inversion noise..), discarded for sub-optimal visuality.
        elif config['edit_mode'] == 'SDEdit': 
            latents = inference(pipe, controlnet, frescoProc, 
                  ...
                  record_latents, propagation_mode,
                  flows = flows, occs = occs, saliency=saliency, repeat_noise=False, 
                  use_inv_noise=config['use_inv_noise'], inv_noise=inv_noise, img_idx=img_idx)
...
def run_tokenflow_translation(config):
